...
def buscaRetanguloPlaca(source):
    ...
    while video.isOpened():
        ...
        alt,lar, _ = frame.shape
        area = frame[int(alt-800):, 150:int(lar-10)]
...
if __name__ == "__main__":
    ...
    numero = numero.strip('\n')
    # print("Leitura da placa 1: " + numero)
    plates.append(numero)

    print(plates)

    for i in range(len(plates)):
        data.append(validate_plate(plates[i], authorized_plate))
    
    for i in range(len(plates)):
        search.append(procurado_plate(plates[i], search_place))
